...
function autocompleteObjectValidator(validOptions: Array<ResourceListType>): ValidatorFn {
...
export class TfrCreationResourceComponent implements OnInit {
  ...
  public validation_msgs = {
    'resource_name': [
      { type: 'invalidAutocompleteObject', message: 'Resource name not recognized. Click one of the autocomplete options.' },
      { type: 'required', message: 'Resource is required.' }
    ],
    'role': [
      { type: 'invalidAutocompleteString', message: 'Role not recognized. Click one of the autocomplete options.' },
  ...
  ngOnInit(): void {
    ...
    this.resourceFormGroup = new FormGroup({
      resource_name: new FormControl('', {validators: [autocompleteObjectValidator(this.resources)]}),
      role: new FormControl('', {validators: [autocompleteStringValidator(this.roles),Validators.required]})
    });
    this.filteredRoleOption = this.resourceFormGroup.controls['role'].valueChanges.pipe(
      startWith(''),
      map(value => this._filterRole(value || '')),
    );

    this.filteredResourceOption = this.resourceFormGroup.controls['resource_name'].valueChanges.pipe(
      startWith(''),
  ...
  private _filterResource(value: string) : ResourceListType[] {
    const filterValue = value.toLowerCase();
    return this.resources.filter(resource =>!resource.selected).filter(resource=>resource.resource_name.toLowerCase().includes(filterValue));
  }

  addResource(resource_name: string, role: string){
    const index = this.resources.findIndex(resource => resource.resource_name===resource_name);
  ...
  removeResource(resource_id: number){
    const indexForResourcesArr = this.resources.findIndex(resource => resource.resource_id===resource_id);
    this.resources[indexForResourcesArr].selected=false;
    const indexForAllocatedResourceArr = this.allocatedResources.findIndex(resource => resource.resource_id===resource_id);
